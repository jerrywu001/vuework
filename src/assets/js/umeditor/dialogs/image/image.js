...
//                }
        ...
        callback: function (editor, $w, url, state) {
            ...
            if (state == "SUCCESS") {
                //显示图片计数+1
                Upload.showCount++;
                var $img = $("<img src='" + url + "' class='edui-image-pic' />"),		//editor.options.imagePath + url
    ...
    var Upload = {
        ...
        uploadComplete: function(r){
            ...
            try{	//改动点
                var json = eval('('+r+')');
                json.state = json.status == 1000? 'SUCCESS': '';
                json.url = !!json.data.key? json.data.key: '';
                Base.callback(me.editor, me.dialog, 'http://7xkvpt.com1.z0.glb.clouddn.com/' + json.url, json.state);
            }catch (e){
        ...
        submit: function (callback) {
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
                ...
                $('<iframe name="up"  style="display: none"></iframe>').insertBefore(me.dialog).on('load', function(){
                    var r = (this.contentWindow.document.body.firstElementChild||this.contentWindow.document.body).innerHTML;
                    if(r == '')return;
                    me.uploadComplete(r);
                    $(this).unbind('load');
                    $(this).remove();
